...
Dataset: $REPLICA_ROOT/<scene>/{audio_wav,erp_depth}/*  (18 scenes,
...
Spec recipe (matches MP3D's data.py so the model input format is identical across datasets):
  ...
  nearest-resized to (256,512). WINDOW = round-trip window for MAX_DEPTH (279.9 samples/m @48k at
  343 m/s; =2799 at 10 m. MP3D keeps its legacy 2823-sample window). Depth (512,1024) is nearest-resized to (256,512); mask = depth>0 & finite.
"""
...
_SPEC_CACHE = os.environ.get("REPLICA_SPEC_CACHE", "")   # output of build_spec_cache_replica.py (fp32, bit-identical to on-the-fly)
