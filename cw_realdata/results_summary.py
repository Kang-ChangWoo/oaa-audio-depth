...
for scene in ("room","corner","hall"):
    ...
    for i,(mk,mn) in enumerate(MODELS):
        for j,(dk,dn) in enumerate(DS):
            p=np.load(os.path.join(R,scene,f"pred_{dk}_{mk}.npy"))
    ...
    fig.tight_layout(); fig.savefig(os.path.join(R,scene,"summary.png"),dpi=110)
    # horizon overlay
    ...
    ax.set_title(f"{scene}: horizon profiles"); fig2.tight_layout(); fig2.savefig(os.path.join(R,scene,"summary_horizon.png"),dpi=110)
    print(f"== {scene}")
