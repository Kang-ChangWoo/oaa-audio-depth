...
HIGHER={'delta1'}
CELLS=[
 ('Replica 2ch',rep,[('OAA-CNN','oaa_r2_fin'),('EchoDiffusion','es_r2_fin'),('eat+LLRD','0820_eatllrd_r2_rep'),('sslam','0820_sslam_r2_rep'),('sslam+LLRD','0820_sslam_llrd_r2_rep')]),
 ('Replica 4ch',rep,[('OAA-CNN','oaa_fb_fin'),('EchoDiffusion','es_fb_fin'),('eat+LLRD','0820_eatllrd_fb_rep'),('sslam s0','0820_sslam_fb_rep'),('sslam s1','0820_sslam_s1_fb_rep'),('sslam s2','0820_sslam_s2_fb_rep'),('sslam+LLRD','0820_sslam_llrd_fb_rep')]),
 ('Replica 6ch',rep,[('OAA-CNN','oaa_r6_fin'),('EchoDiffusion','es_r6_fin'),('eat+LLRD','0820_eatllrd_r6_rep'),('sslam s0','0820_sslam_r6_rep'),('sslam s1','0820_sslam_r6_rep_s1'),('sslam s2','0820_sslam_r6_rep_s2'),('sslam+LLRD','0820_sslam_llrd_r6_rep')]),
 ('Replica 8ch',rep,[('OAA-CNN','oaa_r8_fin'),('EchoDiffusion','es_r8_fin'),('eat+LLRD vd','0820_eatllrd_r8_rep'),('sslam vd','0820_sslam_r8vd_rep'),('sslam+LLRD vd','0820_sslam_llrd_r8vd_rep')]),
 ('MP3D 2ch',mp,[('OAA-CNN','oaa_r2_fin'),('EchoDiffusion','es_r2_fin'),('eat+LLRD','0820_eatllrd_r2_mp3d'),('eat+LLRD+cs','0820_eatllrd_cs_r2_mp3d'),('sslam','0820_sslam_r2_mp3d'),('sslam+LLRD','0820_sslam_llrd_r2_mp3d')]),
 ('MP3D 4ch',mp,[('OAA-CNN','oaa_fb_fin'),('EchoDiffusion','es_fb_fin'),('eat+LLRD s0','0820_eat_llrd_fb_mp3d'),('sslam','0820_sslam_fb_mp3d'),('sslam+LLRD65','0820_sslam_llrd65_fb_mp3d')]),
 ('MP3D 6ch',mp,[('OAA-CNN','oaa_r6_fin'),('EchoDiffusion','es_r6_fin'),('eat+LLRD','0820_eatllrd_r6_mp3d'),('eat+LLRD+vd','0820_eatllrd_r6vd_mp3d')]),
 ('MP3D 8ch',mp,[('OAA-CNN','oaa_r8_fin'),('EchoDiffusion','es_r8_fin'),('eat+LLRD novd s0','0820_eatllrd_r8novd_mp3d'),('eat+LLRD vd','0820_eatllrd_r8_mp3d')]),
]
rows=[]
rows.append('<!--APPENDIX_A-->')
rows.append('<h2>부록 A. 칸별 전 지표 비교표</h2>')
rows.append('<p>본문 표는 판정 지표(MAE)만 싣지만, MAE가 동률인 칸에서도 보조 지표는 체계적으로 갈린다. 아래 표는 여덟 평가 칸 각각에서 판정 대상 모델들의 전 지표를 같은 eval 파이프라인으로 정렬한 것이다(굵게 = 해당 칸·해당 지표 1위; δ1은 높을수록, 나머지는 낮을수록 좋음). 다시드 설정은 시드별 행을 그대로 실어 분산을 감추지 않았다. 표가 보여주는 반복 구조는 세 가지다. 첫째, 근거리(&lt;3m) 열은 거의 모든 칸에서 CNN이 1위이고, 원거리(&gt;6m) 열은 AFM이 일관되게 1위다 — MAE 동률의 내부는 "CNN=근거리 정밀, AFM=원거리·큰 오차 억제"의 교환이며, 이는 §6의 초기 국소 단서 대 분산 잔향 활용 그림과 같은 방향이다(Replica 8ch: far 1.395 vs 1.589, −12%). 둘째, 유일한 예외가 Replica 6ch 시드 0으로, 근거리까지 포함해 일곱 지표 전부에서 1위인 캠페인 유일 사례다. 셋째, EchoDiffusion은 전 칸·전 지표에서 열세이고, MP3D 6ch은 CNN이 MAE만이 아니라 지표 전반에서 방어하는 유일한 칸이다. MP3D 8ch의 vdrop 판(0.9858)은 전 지표 붕괴로, "vdrop은 MP3D에서 전면 기각" 법칙의 극단 사례다. eat+LLRD의 다시드 칸(MP3D 4/8ch)은 지면상 시드 0 행만 실었고 판정은 본문대로 시드 평균을 따른다.</p>')
