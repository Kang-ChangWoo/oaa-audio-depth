...
vals = [(l, v, c) for l, v, c in vals if v]
ys = range(len(vals))
...
ax.set_xlim(0.6, 1.30); ax.set_xlabel("MP3D 8ch  best val MAE (m)")
ax.set_title("캠페인 유일의 패배 칸에 시도한 모든 개입\n회색=무효 · 파랑=fusion 구조 · 초록=입력", fontsize=10.5)
...
bx.set_xlim(-0.30, 0.03)
bx.set_xlabel("Δ best val MAE:  hop 44 − hop 160      ← hop 44 우세")
