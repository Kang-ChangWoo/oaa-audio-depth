...
REQUIRES the isolated conda env + weights (this will NOT run in the base env):
    ...
    Weights (HuggingFace cache):
        facebook/wav2vec2-base-960h  ->  $HF_HOME (HuggingFace cache)
...
# HF cache for `facebook/wav2vec2-base-960h`: set HF_HOME yourself; the local default below is only
# applied when that directory exists on this machine.
_HF_DEFAULT = os.environ.get("ECHODIFF_WEIGHTS", "")   # optional local weights dir
if _HF_DEFAULT and os.path.isdir(_HF_DEFAULT):
    os.environ.setdefault("HF_HOME", _HF_DEFAULT)
...
class CIDE(nn.Module):
    ...
    def forward(self, x, cached=None):
        # cached (B,768): precomputed wav2vec2 embeddings (CIDE caching, 2026-07-25). Justification: the
        # output of self.conv only enters a no_grad block, so it receives no gradient -> conv is a dead,
        # randomly-initialised frozen parameter (quirk of the original repo) and wav2vec2 is frozen too ->
        # the whole path is a fixed function, so caching is lossless. The cache is built with a canonical
        # channel-mean mixing instead of conv (build_cide_cache.py).
        if cached is not None:
            class_probs = self.m(self.fc(cached))
            class_embeddings = class_probs @ self.embeddings
            return self.embedding_adapter(class_embeddings, self.gamma)
...
class EchoDiffusionEncoder(nn.Module):
    ...
    def forward(self, audio_spec, audio_wave, cide=None):
        ...
        if self.cide_module is None:                  # wave_mode="none" ablation
            ...
            conditioning_scene_embedding = self.cide_module(audio_wave, cached=cide)
...
class EchoDiffusionDepth(nn.Module):
    ...
    def __init__(self, in_ch=2, wave_mode="cide", wave_ch=2, faithful=False):
        super().__init__()
        self.in_ch = in_ch
        self.wave_ch = wave_ch
        # faithful=True (2026-07-25, match the original code): like the original repo, squash the spec to a
        # 128x128 square and bilinearly upsample the 256x256 output to 256x512 afterwards. Only the crash-bug
        # fix is kept; the advantages we had added (128x256 encoding + native 256x512 decoding) are removed.
    ...
    def forward(self, spec, wave, cide=None):
        # faithful: original repo's 128x128 square resize (breaks aspect) / enhanced: aspect-preserving 128x256 resize
        enc_hw = (128, 128) if self.faithful else _ENC_HW
        if spec.shape[-2:] != enc_hw:
            spec_in = F.interpolate(
                spec, size=enc_hw, mode="bilinear", align_corners=False
            )
        else:
            spec_in = spec

        conv_feats = self.encoder(spec_in, wave, cide=cide)   # (B,1536,8,16) | faithful: (B,1536,8,8)
        out = self.decoder([conv_feats])           # (B,192,256,512) | faithful: (B,192,256,256)
        out_depth = self.last_layer_depth(out)
        if self.faithful and out_depth.shape[-2:] != (256, 512):
            out_depth = F.interpolate(out_depth, size=(256, 512), mode="bilinear", align_corners=False)  # post-hoc upsample
