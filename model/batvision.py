...
class RotDepth(nn.Module):
    ...
    NOTE (kept for checkpoint compatibility): the U-Net is built with output_nc=feat_c (32) and depth_norm=True,
    so its outermost block applies a Sigmoid to the 32-ch feature map before the 3x3 head + second sigmoid.
    The original BatVision has output_nc=1 (single sigmoid = depth). Parameter count is unaffected (54.5 M)."""
