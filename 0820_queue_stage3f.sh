#!/bin/bash
# 0820 Stage-3f: convstem + eat_llrd combination — attack the near-field gap (sub-patch locality)
# while keeping the LLRD recipe that fixed the post-norm LR spike. Replica fb + MP3D fb.
cd "$(dirname "$0")"
...
LL="--audio-backbone eat --afm-llrd 0.75 --warmup-ep 8 --afm-stem conv"
SEEDS="${SEEDS:-0}"
GPUS="${GPUS:-0 1 2 3 4 5 6 7}"
JOBS=(
"0820_eatllrd_cs_fb_rep|data_0422|$LL --nviews 4 --data-mode fb --epochs 40 --batch-size 8 --accum 4"
"0820_eatllrd_cs_fb_mp3d|data_mp3d|$LL --nviews 4 --data-mode fb --epochs 40 --batch-size 8 --accum 4"
)
SJOBS=()
for s in $SEEDS; do
  for j in "${JOBS[@]}"; do
...
echo "[dispatch] all ${#SJOBS[@]} stage-3f jobs launched"
